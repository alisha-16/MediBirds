...
if($chk == '1')
{
    $sql = "DELETE from surgical WHERE Sname = '$_POST[Name]'";
...
{
    $sql = "DELETE from medical WHERE Mname = '$_POST[Name]'";
    if($link->query($sql)=== true)
    {
        echo "<br> Medical Product record deleted ";
...
if($chk == '3')
{
    $sql = "DELETE from covid WHERE Cname = '$_POST[Name]'";
    if($link->query($sql)=== true)
    {
        echo "<br> Covid Product record deleted ";
    }
    else
    {
        echo "ERROR : could not execute  sql "  . $link->error."<br>";
